        ...
        if (user) {
          ...
          if (check) {
            ...
            let tokenAccess = jwt.sign(
              ...
              process.env.JSON_SECRET_ACCESS,
              {
                expiresIn: "30m",
              }
            );
            let tokenRefresh = jwt.sign(
              { id: user.id, roleId: user.roleId },
              process.env.JSON_SECRET_REFRESH,
              {
                expiresIn: "1h",
              }
            );
    ...
    try {
      ...
      if (check) {
        resolve({
          errCode: 1,
          errMessage:
            "Your email is already in used. Plz check lai ho minh nha",
        });
      } else {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!data.token) {
        ...
        jwt.verify(
          data.token,
          process.env.JSON_SECRET_REGISTER,
          async (err, decoded) => {
            if (err) {
              resolve({
                errCode: 2,
    ...
    try {
      if (!data.email) {
        ...
        if (user) {
          let token = jwt.sign(
            { email: data.email },
            process.env.JSON_SECRET_FORGOT_PASSWORD
          );
